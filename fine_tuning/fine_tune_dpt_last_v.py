"""
Config-driven fine-tuning of TabDPT's last-layer `W_V` weights.

This mirrors the rolling one-step tuning loop from `fine_tune_dpt.py`, but it
always runs without text attention. Only the final transformer's value
projection is updated:

- the value half of `kv_proj.weight` (`W_V`)

The query projection `q_proj.weight` (`W_Q`) and the key half of
`kv_proj.weight` (`W_K`) remain frozen.

The script reuses the existing YAML schema from `fine_tune_dpt.py`. In
particular, `tuning.gate_lr` is used as the optimizer learning rate for V
...
def get_last_layer_v_param(reg: TabDPTRegressor) -> torch.nn.Parameter:
    ...
    return last_block.kv_proj.weight


def freeze_all_but_last_v(
    reg: TabDPTRegressor,
) -> tuple[torch.nn.Parameter, list[torch.nn.Parameter], torch.utils.hooks.RemovableHandle]:
    """Freeze all parameters except the last block's `W_V`."""
    for p in reg.model.parameters():
        p.requires_grad_(False)

    last_block = _get_last_block(reg)
    kv_weight = get_last_layer_v_param(reg)
    kv_weight.requires_grad_(True)

    value_mask = torch.zeros_like(kv_weight)
    value_mask[last_block.embed_dim :] = 1.0
    hook = kv_weight.register_hook(lambda grad, mask=value_mask: grad * mask)
...
def get_training_info(reg: TabDPTRegressor) -> str:
    ...
    return " | ".join(
        [
            ...
            _weight_stats("W_K(frozen)", k_weight),
            _weight_stats("W_V", v_weight),
        ]
    )


def fine_tune_last_v(
    reg: TabDPTRegressor,
    *,
    ...
    """
    Fine-tune only the last layer's regular-attention `W_V`.
    ...
    """
    _, tunable_params, kv_hook = freeze_all_but_last_v(reg)
    optimizer, optimizer_name = _build_optimizer(tunable_params, lr=tuning_cfg.gate_lr)
    print(
        f"Tuning last-layer W_V (loss_type={tuning_cfg.loss_type}, "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fine-tune last-layer W_V on a configured dataset without text attention."
    )
