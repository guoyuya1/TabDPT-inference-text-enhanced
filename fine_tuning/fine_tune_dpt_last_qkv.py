"""
Config-driven fine-tuning of TabDPT's last-layer `W_Q`, `W_K`, and `W_V` weights.

This mirrors the rolling one-step tuning loop from `fine_tune_dpt.py`, but it
always runs without text attention. Only the final transformer's regular
attention projections are updated:
...
particular, `tuning.gate_lr` is used as the optimizer learning rate for Q/K/V
...
def freeze_all_but_last_qkv(
    reg: TabDPTRegressor,
) -> tuple[torch.nn.Parameter, torch.nn.Parameter, list[torch.nn.Parameter]]:
    """Freeze all parameters except the last block's `W_Q`, `W_K`, and `W_V`."""
    ...
    return q_weight, kv_weight, [q_weight, kv_weight]


def get_training_info(reg: TabDPTRegressor) -> str:
    """Compact one-line summary for the trainable last-layer Q/K/V weights."""
    last_block = _get_last_block(reg)
    q_weight = last_block.q_proj.weight
...
def fine_tune_last_qkv(
    ...
    *,
    ...
    """
    Fine-tune only the last layer's regular-attention `W_Q`, `W_K`, and `W_V`.
    ...
    """
    _, _, tunable_params = freeze_all_but_last_qkv(reg)
    optimizer, optimizer_name = _build_optimizer(tunable_params, lr=tuning_cfg.gate_lr)
    print(
        f"Tuning last-layer W_Q/W_K/W_V (loss_type={tuning_cfg.loss_type}, "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fine-tune last-layer W_Q/W_K/W_V on a configured dataset without text attention."
    )
