    ...
    *,
    ...
    with torch.no_grad():
        for lag_idx in range(lags):
            train_lag = text_train_step[:, lag_idx, :].to(device)
            test_lag =text_test_step[:, lag_idx, :].to(device)
            ...
            pca_train_parts.append(train_proj.detach().cpu())
            pca_test_parts.append(test_proj.detach().cpu())

    X_train_aug = torch.cat([X_train_step] + pca_train_parts, dim=1)
    X_test_aug = torch.cat([X_test_step] + pca_test_parts, dim=1)
    ...
    *,
    ...
    with torch.no_grad():
        for idx in range(len(y_eval)):
            X_train_full = torch.cat((X_context_proc, X_eval_proc[:idx]))
            y_train_full = torch.cat((y_context, y_eval[:idx]))
            if use_text:
                text_train_full = torch.cat((text_context, text_eval[:idx]), dim=0)
            ...
            y_context_tensor = y_train_step.unsqueeze(0).to(reg.device)
    ...
    mae, rmse, mape = _compute_metrics(y_eval.numpy(), preds)
    ...
    *,
    ...
    with torch.no_grad():
        for idx in range(len(y_eval)):
            ...
            X_train_tensor = X_train_aug.to(device=reg.device).unsqueeze(0)
            X_train_tensor = pad_x(X_train_tensor, reg.max_features)
            X_test_tensor = X_test_aug.to(device=reg.device).unsqueeze(0)
            X_test_tensor = pad_x(X_test_tensor, reg.max_features)
            y_context_tensor = y_train_step.to(device=reg.device).unsqueeze(0)
