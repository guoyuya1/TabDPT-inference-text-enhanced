"""
Config-driven fine-tuning of TabDPT's last-layer `W_Q` weights.

This mirrors the rolling one-step tuning loop from `fine_tune_dpt.py`, but it
always runs without text attention. Only the final transformer's query
projection is updated:

- `q_proj.weight` (`W_Q`)

The key and value projections remain frozen.

The script reuses the existing YAML schema from `fine_tune_dpt.py`. In
particular, `tuning.gate_lr` is used as the optimizer learning rate for Q-only
...
def freeze_all_but_last_q(reg: TabDPTRegressor) -> tuple[torch.nn.Parameter, list[torch.nn.Parameter]]:
    """Freeze all parameters except the last block's `W_Q`."""
...
def fine_tune_last_q(
    ...
    *,
    ...
    """
    Fine-tune only the last layer's regular-attention `W_Q`.
    ...
    """
    _, tunable_params = freeze_all_but_last_q(reg)
    optimizer, optimizer_name = _build_optimizer(tunable_params, lr=tuning_cfg.gate_lr)
    print(
        f"Tuning last-layer W_Q (loss_type={tuning_cfg.loss_type}, "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fine-tune last-layer W_Q on a configured dataset without text attention."
    )
