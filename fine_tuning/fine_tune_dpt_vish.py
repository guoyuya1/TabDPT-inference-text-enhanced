...
#Importing from my own modified version of TabDPT
from tabdpt_vish import TabDPTRegressor
from tabdpt_vish.utils import pad_x, convert_to_torch_tensor

try:
    from .eval_fine_tune_vish import _format_metrics, evaluate_rolling, evaluate_rolling_pca
    ...
    for epoch in range(1, tuning_cfg.epochs + 1):
        for step_idx in range(num_steps):
            ...
            for point_idx in range(start, end):
                X_point_context = torch.cat((X_context_proc, X_tune_proc[:point_idx]))
                y_point_context = torch.cat((y_context, y_tune[:point_idx]))
                text_point_context = torch.cat((text_context, text_tune[:point_idx]), dim=0)
                ...
                #TODO: Vish change, remove text attention for now
                # attn_weight_external = reg._compute_attn_weight_pairwise_avg(train_text_batch, test_text_batch)
                attn_weight_external = None

                X_train_tensor = X_context_step.unsqueeze(0).to(reg.device)
                X_test_tensor = X_tune_proc[point_idx:point_idx + 1].unsqueeze(0).to(reg.device)
                ...
                y_point_target = y_tune[point_idx:point_idx + 1].to(reg.device)
        ...
        if tuning_cfg.eval_each_epoch:
            ...
            eval_context_proc = torch.cat((X_context_proc, X_tune_proc), dim=0)
            eval_y_context = torch.cat((y_context, y_tune))
            eval_text_context = torch.cat((text_context, text_tune), dim=0)
...
def main() -> None:
    ...
    X_context_proc = torch.tensor(X_context_proc, dtype=torch.float32)
    y_context = torch.tensor(y_context, dtype=torch.float32)
    text_context = torch.tensor(text_context, dtype=torch.float32)
    X_tune_proc = torch.tensor(X_tune_proc, dtype=torch.float32)
    y_tune = torch.tensor(y_tune, dtype=torch.float32)
    text_tune = torch.tensor(text_tune, dtype=torch.float32)
    X_eval_proc = torch.tensor(X_eval_proc, dtype=torch.float32)
    y_eval = torch.tensor(y_eval, dtype=torch.float32)
    text_eval = torch.tensor(text_eval, dtype=torch.float32)
    
    eval_context_proc = torch.cat((X_context_proc, X_tune_proc), dim=0)
    eval_y_context = torch.cat((y_context, y_tune), dim=0)
    eval_text_context = torch.cat((text_context, text_tune), dim=0)
