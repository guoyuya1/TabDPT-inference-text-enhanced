...
def _resolve_date_col(df: pd.DataFrame, requested_col: str) -> str:
    ...
    for fallback in fallbacks.get(requested_col, []):
        if fallback in df.columns:
            return fallback

    raise ValueError(f"Missing date column '{requested_col}'")
...
def _read_text_csv(
    ...
    *,
    ...
    start_date_col = _resolve_date_col(df, start_date_col)
    end_date_col = _resolve_date_col(df, end_date_col)
...
def _merge_with_interval_overlap(
    *,
    ...
    numeric_start_date_col = _resolve_date_col(numeric, numeric_start_date_col)
    numeric_end_date_col = _resolve_date_col(numeric, numeric_end_date_col)
    ...
    for _, row in numeric.iterrows():
        ...
        search_rows.append(
            _collect_overlapping_text(
                ...
                text_df=search,
                text_start_col=_resolve_date_col(search, text_start_date_col),
                text_end_col=_resolve_date_col(search, text_end_date_col),
        ...
        report_rows.append(
            _collect_overlapping_text(
                ...
                numeric_end=numeric_end,
                text_df=report,
                text_start_col=_resolve_date_col(report, text_start_date_col),
                text_end_col=_resolve_date_col(report, text_end_date_col),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Join Health_AFR numeric + text CSVs by interval overlap."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--numeric",
        default="Time-MMD/numerical/Health_AFR/Health_AFR.csv",
        help="Path to numeric CSV.",
    )
    parser.add_argument(
        "--search",
        default="Time-MMD/textual/Health_AFR/Health_AFR_search.csv",
        help="Path to search-summary text CSV.",
    )
    parser.add_argument(
        "--report",
        default="Time-MMD/textual/Health_AFR/Health_AFR_report.csv",
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="Time-MMD/numerical/Health_AFR/Health_AFR_with_text.csv",
